...
app.post('/add', async (req, res) => {
  ...
  const alreadyExists = data.find(item => item === number);
  if (alreadyExists) {
...
app.post('/delete', authMiddleware, async (req, res) => {
  ...
  data = data.filter(item => item !== number);
  await updateData(data);
  req.session.message = 'Berhasil menghapus nomor ✓';
  res.redirect('/admin');
});

// Endpoint RAW JSON dari GitHub API
...
// Endpoint /deletenumber?number=
app.get('/deletenumber', async (req, res) => {
  const { number } = req.query;
  let data = await fetchData();
  const exists = data.some(item => item === number);
